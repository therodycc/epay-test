...
export const getAllClient = () => {
  return async (dispatch: AppDispatch) => {
    const res: any = await clientProvider.getAll();
    if (!res || res?.error) {
...
export const createClient = (
  ...
  return async (dispatch: AppDispatch) => {
    const res:any = await clientProvider.create(client);
    if (!res || res?.error) {
...
export const updateClient = (
  ...
  return async (dispatch: AppDispatch) => {
    const res:any = await clientProvider.updateData(id.toString(), data);
    if (!res || res?.error) {
      alert("Error creando cliente");
      return;
    }
...
export const removeClient = (uuid: string) => {
  return async (dispatch: AppDispatch) => {
    const res:any = await clientProvider.remove(uuid);
    if (!res || res?.error) {
      alert("Error creando cliente");
      return;
    }
